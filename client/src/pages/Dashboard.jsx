...
const Dashboard = () => {
  ...
  const fetchClasses = async (userId) => {
    try {
      ...
      const res = await axios.get(`http://localhost:10000/api/classes/${userId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role === 'TEACHER') {
        await axios.post('http://localhost:10000/api/classes/create', { ...formData, teacherId: user._id }, config);
        alert('Classroom Created Successfully!');
      } else {
        await axios.post('http://localhost:10000/api/classes/join', { code: formData.code, studentId: user._id }, config);
