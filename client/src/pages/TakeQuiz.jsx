...
const TakeQuiz = () => {
  ...
  useEffect(() => {
    ...
    const fetchQuiz = async () => {
      try {
        ...
        const res = await axios.get(`http://localhost:10000/api/quizzes/single/${quizId}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const userInfo = JSON.parse(localStorage.getItem('userInfo'));
      const token = userInfo ? userInfo.token : null;
      const res = await axios.post('http://localhost:10000/api/quizzes/submit', { quizId, studentId: user._id, answers }, { headers: { Authorization: `Bearer ${token}` } });
